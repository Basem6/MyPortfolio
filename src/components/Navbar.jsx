...
function  scrollToSection(href , pos) {
  ...
  if (smoother) {
    if(href!="#contact"){
    smoother.scrollTo(href,true ,pos)
    }else{
      const maxScroll =
...
export default function Navbar() {
  ...
  const handleNavClick = (e, href) => {
    e.preventDefault()
    scrollToSection(href , "bottom bottom")
  }

  // FIX: close mobile menu AND scroll on the same click
  const handleMobileNavClick = (e, href) => {
    e.preventDefault()
    setIsOpen(false)
    scrollToSection(href , "top 10%")
  }
    ...
    >
        <div className="max-w-full px-2 lg:px-10 ">
          <div className="flex items-center justify-between h-10 lg:h-14  max-w-full">
            ...
            <div className="hidden md:flex  gap-2">
              ...
              {/* FIX: was <NavLink to="/#contact"> — same React Router nav issue */}
              <a
                href="https://mail.google.com/mail/?view=cm&fs=1&to=basem556473@gmail.com&su=Hire%20Request&body=Hi%20Basem"
                target="_blank"
        ...
        >
          <div className="px-8 py-6 space-y-2">
            {navLinks.map((link) => (
              // FIX: was <NavLink to={...}> with no close/scroll handler.
              //      Now closes menu AND scrolls to section on click.
              <a
                key={link.name}
                href={link.href}
                onClick={(e) => handleMobileNavClick(e, link.href)}
                className={`block px-5 py-4 rounded-xl transition-colors text-base ${
            ...
            {/* FIX: was <NavLink to="/#contact"> — same issues as above */}
            <a
              href="https://mail.google.com/mail/?view=cm&fs=1&to=basem556473@gmail.com&su=Hire%20Request&body=Hi%20Basem"
              target="_blank"
              className="block mt-4 px-5 py-4 text-center font-light bg-white text-black rounded-xl text-base"
            >
              Hire Me
            </a>
          </div>
